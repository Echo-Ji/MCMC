...
N, D = X_train.shape
layer_size = [D, 40, 40, 1]

# MODEL
model_start = time.time()
C = 2  # expected number of modes in posterior distribution
with tf.variable_scope("W_0"):
    probs = Dirichlet(tf.ones([D, 40, C]))
    mu = Normal(tf.zeros([D, 40]), 40*tf.ones([D, 40]), sample_shape=C)
    sigmasq = InverseGamma(tf.ones([D, 40]), tf.ones([D, 40]), sample_shape=C)
    W_0 = ParamMixture(probs, {'loc': mu, 'scale': 10*tf.sqrt(sigmasq)}, Normal)
with tf.variable_scope("W_1"):
    probs = Dirichlet(tf.ones([40, 40, C]))
    mu = Normal(tf.zeros([40, 40]), 40*tf.ones([40, 40]), sample_shape=C)
    sigmasq = InverseGamma(tf.ones([40, 40]), tf.ones([40, 40]), sample_shape=C)
    W_1 = ParamMixture(probs, {'loc': mu, 'scale': 10*tf.sqrt(sigmasq)}, Normal)
with tf.variable_scope("W_2"):
    probs = Dirichlet(tf.ones([40, 1, C]))
    mu = Normal(tf.zeros([40, 1]), 40*tf.ones([40, 1]), sample_shape=C)
    sigmasq = InverseGamma(tf.ones([40, 1]), tf.ones([40, 1]), sample_shape=C)
    W_2 = ParamMixture(probs, {'loc': mu, 'scale': 10*tf.sqrt(sigmasq)}, Normal)
with tf.variable_scope("b_0"):
    probs = Dirichlet(tf.ones([40, C]))
    mu = Normal(tf.zeros(40), 40*tf.ones(40), sample_shape=C)
    sigmasq = InverseGamma(tf.ones(40), tf.ones(40), sample_shape=C)
...
with tf.variable_scope("b_2"):
    ...
    mu = Normal(tf.zeros(1), 40*tf.ones(1), sample_shape=C)
...
with tf.variable_scope("qW_0"):
    probs = tf.get_variable("probs", [D, 40, C], initializer=tf.constant_initializer(1.0 / C))
    mu = tf.get_variable("loc", [C, D, 40])
    sigma = tf.nn.softplus(tf.get_variable("scale", [C, D, 40]))
    qW_0 = ParamMixture(probs, {'loc': mu, 'scale': sigma}, Normal)
with tf.variable_scope("qW_1"):
    probs = tf.get_variable("probs", [40, 40, C], initializer=tf.constant_initializer(1.0 / C))
    mu = tf.get_variable("loc", [C, 40, 40])
    sigma = tf.nn.softplus(tf.get_variable("scale", [C, 40, 40]))
    qW_1 = ParamMixture(probs, {'loc': mu, 'scale': sigma}, Normal)
with tf.variable_scope("qW_2"):
    probs = tf.get_variable("probs", [40, 1, C], initializer=tf.constant_initializer(1.0 / C))
    mu = tf.get_variable("loc", [C, 40, 1])
    sigma = tf.nn.softplus(tf.get_variable("scale", [C, 40, 1]))
    qW_2 = ParamMixture(probs, {'loc': mu, 'scale': sigma}, Normal)
with tf.variable_scope("qb_0"):
    probs = tf.get_variable("probs", [40, C], initializer=tf.constant_initializer(1.0 / C))
    mu = tf.get_variable("loc", [C, 40])
    sigma = tf.nn.softplus(tf.get_variable("scale", [C, 40]))
...
B = 10000
K = 20
epochs = 5000 # change it to get more iteration
batchsz = 10000
...
# Samples Dumping
dump_time = time.time()
with open('./theta/2h-40-40-10v/'+reg_pre+'fb'+str(C)+'-'+str(J)+'-'+str(B)+'-2h-10v','wb') as f:
