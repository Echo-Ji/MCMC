...
#    st = time.time()
    ...
    pred = 1 / (1 + np.exp(-np.matmul(pred, theta.w[-1]) - theta.b[-1]))
    pred = pred.reshape([-1])
    pred[pred==0] = 1e-6  #deliminate the effect of log(0)
    pred[pred==1] = 1 - 1e-6
#    print('ln time:', time.time() - st)
    return np.mean(np.log(pred)*y + np.log(1-pred)*(1-y))

'''mod = SourceModule("""
__global__ void gLn(float *theta, float *X, float *y)
{
...
def log_LR(pred1, pred2):
    log_proba1 = np.log(np.abs(pred1 - 1 + y))
    log_proba2 = np.log(np.abs(pred2 - 1 + y))
    return np.sum(log_proba1) - np.sum(log_proba2)
...
def MH(theta0, phi_j):
    ...
    for i in range(K):
        ...
        p2 = X
        for w,b in zip(theta.w[:-1],theta.b[:-1]):
            p2 = act_func(np.matmul(p2, w) + b)
        p2 = 1 / (1 + np.exp(-np.matmul(p2, theta.w[-1]) - theta.b[-1]))
        p2 = p2.reshape([-1])
        
        if phi_j * log_LR(p1,p2) + log_prior_ratio(_theta,theta) > thresholds[i]:
            theta = _theta
...
def SMC(samples):
    ...
    for j in range(1,J):
        ...
        #startphi = time.time()
        if ESS > 0.5*B:
            varphi = deepcopy(samples)
        else:
            varphi = draw_from_multinomial(samples, w)
            w = np.ones(shape=B)
        print('phi time:', time.time() - startphi)
        startpool = time.time()
        pool = Pool(32)
        results = []
        #samples = []
        for theta0 in varphi:
            results.append(pool.apply_async(MH, (theta0,phi[j],)))
